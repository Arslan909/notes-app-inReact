...
export default function App() {
	...
	React.useEffect(() => {         //this will set the description State var. to the description of the selected note in the side bar 
		if (selectedNote!== null) {
			noteData.map((note) => {
				if (note[0] === selectedNote) {
					console.log("selected note is "+ note[0]);
					setNoteId(note[0])
					setNoteTile(note[1])
		...
		else if (selectedFolderNote!==null) {
			folders.map(folder => {
				folder.folderNotes.map(ele => {
					if (ele.noteId === selectedFolderNote) {
						console.log("selected folder is "+ ele.noteId);
						console.log(ele);
	...
	//todo- fix the new note is creating array not object!
